...
import com.xently.holla.providers.LocalStorageProvider
import java.io.*
...
object BitmapUtils {
    ...
    /**
     * Converts [Bitmap] to [Base64] encoded string
     */
    fun Bitmap.asString(): String {
        val byteArrayOutputStream = ByteArrayOutputStream()
        this@asString.compress(Bitmap.CompressFormat.JPEG, 100, byteArrayOutputStream)
        val byteArray: ByteArray = byteArrayOutputStream.toByteArray()
        return Base64.encodeToString(byteArray, Base64.DEFAULT)
    }

    /**
     * Scales down a file from [photoFilePath] to fit the dimensions of [view]
     * @return scaled down [Bitmap]
     */
    fun scaledBitmapToMatchView(view: ImageView, photoFilePath: String): Bitmap {
        // Calculate [view]'s height and width
        ...
        val bitmapOptions = BitmapFactory.Options()
            .apply {
                ...
                // Calculate [picture]'s height and width
                val pictureHeight = outHeight
                val pictureWidth = outWidth

                // Determine how much the picture should be scaled down by
                val scaleFactor = min(
        ...
        return BitmapFactory.decodeFile(photoFilePath, bitmapOptions)
    }
}
...
object FileUtils {
    ...
    private val LOG_TAG = FileUtils::class.java.simpleName
     ...
     * @author paulburke
     */
    fun isLocalStorageDocument(uri: Uri): Boolean = LocalStorageProvider.AUTHORITY == uri.authority
    ...
    fun getPath(context: Context, uri: Uri): String? {
        ...
        Log.show(
            LOG_TAG,
            """Authority: ${uri.authority}, Fragment: ${uri.fragment}, Port: ${uri.port}, 
                |Query: ${uri.query}, Scheme: ${uri.scheme}, Host: ${uri.host}, 
                |Segments: ${uri.pathSegments}""".trimMargin()
        )
    ...
    fun getFile(context: Context, uri: Uri?): File? {
        if (uri != null) {
            ...
            if (path != null && isLocal(path)) return File(path)
        }
        return null
    }
    ...
    fun getThumbnail(
        ...
        Log.show(LOG_TAG, "Attempting to get thumbnail")
        ...
        try {
            ...
            if (cursor!!.moveToFirst()) {
                ...
                Log.show(LOG_TAG, "Got thumb ID: $id")

                if (mimeType!!.contains("video")) {
                    // TODO: Switch to the proposed alternative...
                    /*if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        resolver.loadThumbnail(uri, Size(512,384), null)
                    }*/
            ...
            Log.show(LOG_TAG, "getThumbnail", e, Log.Type.ERROR)
        } finally {
            cursor?.close()
        }
        return bm
...
val File.mimeType get() = "application/${this.extension}"